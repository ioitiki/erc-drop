...
import "../ValiantAccessoryFactory.sol";
...
contract TestForReentrancyAttack is IERC1155Receiver {
    ...
    /*function attack(uint256 _totalToMint) external {
        ...
        ValiantAccessoryFactory(factoryAddress).mint(1, address(this), 1, "");
        }*/

    // We attempt a reentrancy attack here by recursively calling the
    // ValiantAccessoryFactory that created the ValiantAccessory ERC1155 token
    // that we are receiving here.
    // We expect this to fail if the ValiantAccessoryFactory.mint() function
        ...
        returns(bytes4)
    {
        ...
        if(balance < totalToMint)
        {
            ...
            ValiantAccessoryFactory(factoryAddress)
